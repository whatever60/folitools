...
# Set script_dir to the directory where this current script is located
...
this_script_path <- function() {
  ...
  for (i in rev(seq_along(frames))) {
    ...
    if (exists("ofile", envir = env, inherits = FALSE)) {
      return(normalizePath(get("ofile", envir = env, inherits = FALSE)))
    }
  }

  stop("Could not determine script path: no 'ofile' found. Is this code being run via source()?")
}

this_script_dir <- function() {
  dirname(this_script_path())
}

this_dir <- this_script_dir()
source(file.path(this_dir, "NPmatch.R"))
source(file.path(this_dir, "normalize.log2CPM.R"))
library(limma)
