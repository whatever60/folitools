...
def raw_read_counts(
    ...
    """Per-gene read counts without UMI deduplication.

    One row per read_id in group.tsv.gz (see uniqueness NOTE in
    ``process_count_file_simple``), so aggregating row counts per gene
    yields pre-dedup read counts.
    """
    gene_counts = group_count.group_by("gene").agg(pl.len().alias("umi_count"))
    if isinstance(gene_counts, pl.LazyFrame):
        gene_counts = gene_counts.collect()
    gene_counts = (
        gene_counts.select(["gene", "umi_count"]).to_pandas().set_index("gene")
...
def process_count_file_simple(
    fps: list[str], dedup_umi: bool = True
) -> pd.DataFrame:
    """
    ...
    Args:
        ...
        dedup_umi: If True (default), collapse reads sharing the same
            final_umi within a gene assignment and primer pair into one count.
            If False, return the raw per-gene read count.
    ...
    for fp in fps:
        ...
        if header == expected:  # A table generated by `umi_tools group`
            full = pl.scan_csv(fp, separator="\t")

            # NOTE: In the foli pipeline (STAR -> featureCounts ->
            # foli_add_tags -> sambamba sort -> umi_tools group) every
            # QNAME contributes 0 or 1 rows to group.tsv.gz, so read_id
            # is strictly unique. The invariant comes from:
            #   1. foli_add_tags.finalize_group downgrades any extra
            #      primary alignment to secondary, guaranteeing <=1
            #      primary per mate per QNAME.
            #   2. umi_tools' bundle iterator (sam_methods.py) yields
            #      R2, unmapped, half-mapped, chimeric and unpaired
            #      alignments as `single_read` -> BAM only, never tsv.
            #   3. Secondary R1s carry no XF tag (foli_add_tags only
            #      stamps XF on primary R1), so umi_tools skips them
            #      via its "Read skipped, no tag" branch.
            # Changes that would break this: skipping foli_add_tags,
            # stamping XF on secondary R1s, running umi_tools on a
            # non-foli BAM, or switching umi_tools bundle keys (e.g.
            # --per-contig) in a way that re-emits the same read_id.
            # Downstream aggregations (dedup / raw counts) assume one
            # row per read, so a violation would silently double-count.
            uniq = full.select(
                pl.col("read_id").n_unique().alias("unique"),
                pl.len().alias("total"),
            ).collect()
            n_unique, n_total = uniq["unique"][0], uniq["total"][0]
            assert n_unique == n_total, (
                f"read_id must be unique in {fp}: {n_total} rows, "
                f"{n_unique} distinct read_ids"
            )

            # Keep the full XF assignment, including primers, during deduplication.
            df = full.filter(~pl.col("gene").str.starts_with("Unassigned,"))
            if dedup_umi:
                gene_counts = deduplicate_umi(df)
            else:
                gene_counts = raw_read_counts(df)
            s = gene_counts["umi_count"]
            s.index = s.index.str.replace(r",[^,]*$", "", regex=True).str.replace(
                ",", "|", regex=False
            )
    ...
    for fp in fps:
        ...
        if header == expected:  # A table generated by `umi_tools group`
            series_list.append(
                ...
                .filter(~pl.col("gene").str.starts_with("Unassigned,"))
...
def read_counts(
    ...
    # process each file
    matrix = process_count_file_simple(files, dedup_umi=dedup_umi)
