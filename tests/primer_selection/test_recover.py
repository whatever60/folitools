...
class TestSanityChecks:
    ...
    def test_sanity_check_primer_duplicates_no_duplicates(self, sample_primer_info, caplog):
    ...
    def test_sanity_check_multi_location_binding_none(self, caplog):
        ...
        assert "✅ No primers bind to multiple locations on the same transcript" in caplog.text
    ...
    def test_sanity_check_sequence_lengths_match(self, caplog):
        ...
        assert "✅ All primer sequence lengths match their locate positions" in caplog.text
    ...
    def test_sanity_check_primer_pair_relationships_good(self, caplog):
        ...
        grouped_df = pd.DataFrame({
            'primer_seq_fwd': ['SEQ1', 'SEQ2'],
            'primer_seq_rev': ['SEQ3', 'SEQ4']
        })
...
class TestHelperFunctions:
    ...
    def test_enrich_locate_df(self, sample_locate_df, sample_primer_info):
        ...
        expected_cols = [
            'primer_seq', 'primer_seq_full', 'primer_type', 'transcript_id',
            'gene_id', 'gene_symbol', 'start', 'end', 'strand', 'pool'
        ]
        assert list(result.columns) == expected_cols
        assert len(result) == 2
    ...
    def test_group_pairs_empty_input(self):
        ...
        expected_cols = [
            'primer_seq_fwd', 'primer_seq_rev', 'transcript_id', 'gene_id',
            'gene_symbol', 'start_up', 'end_up', 'start_down', 'end_down',
            'pool_fwd', 'pool_rev', 'num_transcripts', 'num_genes', 'transcript_id_all'
        ]
        assert list(result.columns) == expected_cols
        assert len(result) == 0
    ...
    def test_build_summary_df_empty_input(self):
        ...
        expected_cols = [
            'Group', 'geneSymbol', 'geneID', 'Chosen Index', 'amplicon_index',
            'L_seq', 'R_seq', 'primer_sequence_to_order_forward',
            'primer_sequence_to_order_reverse', 'L_pool', 'R_pool', 'multi_mapping'
        ]
        assert list(result.columns) == expected_cols
        assert len(result) == 0
