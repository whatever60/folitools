#!/usr/bin/env python3
"""
Test script to verify IDT ordering functionality with multiple pools.
"""
...
def test_multiple_pools():
    """Test the IDT ordering functionality with more than 384 primers."""

    print("Creating test data for multiple pools...")
    temp_dir = tempfile.mkdtemp()

    # Create 200 genes (400 primers total) to test pool splitting
    num_genes = 200
    gene_names = [f"GENE_{i:03d}" for i in range(num_genes)]
    groups = [f"group_{i % 5}" for i in range(num_genes)]  # 5 different groups
    ...
    primer_info_data = pd.DataFrame(
        {
            ...
            "pool": [f"pool{i % 3}" for i in range(num_genes)],  # 3 different pools
        }
    )
    ...
    try:
        ...
        if os.path.exists(output_idt):
            ...
            # Verify that primers for the same gene are in the same pool
            print("\nVerifying gene-pool consistency...")
            gene_pool_check = True
            for i in range(
                0, len(idt_data), 2
            ):  # Check every pair (forward and reverse)
                if i + 1 < len(idt_data):
                    pool1 = idt_data.iloc[i]["Pool name"]
                    pool2 = idt_data.iloc[i + 1]["Pool name"]
                    if pool1 != pool2:
                        print(
                            f"✗ Error: Gene primers at positions {i} and {i + 1} are in different pools"
                        )
                        gene_pool_check = False
                        break

            if gene_pool_check:
                print("✓ All gene primer pairs are in the same pool")
