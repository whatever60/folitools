...
class TestUtilityFunctions:
    ...
    def run_bash_function(self, scripts_dir: Path, function_name: str, args: List[str]) -> Tuple[int, str, str]:
    ...
    def test_is_fastq_file(self, scripts_dir):
        ...
        for filename, expected_exit_code in test_cases:
            ...
            assert exit_code == expected_exit_code, f"Failed for {filename}: expected {expected_exit_code}, got {exit_code}"
    ...
    @pytest.mark.parametrize("input_file,expected_sample", [
    ...
    def test_extract_sample_name(self, scripts_dir, input_file, expected_sample):
        ...
        assert stdout == expected_sample, f"Expected '{expected_sample}', got '{stdout}'"
    
    @pytest.mark.parametrize("r1_file,expected_r2", [
        ...
        ("file_name_with_underscores_R1.fastq.gz", "file_name_with_underscores_R2.fastq.gz"),
    ])
    ...
    def test_validate_file_formats(self, scripts_dir):
        ...
        exit_code, stdout, stderr = self.run_bash_function(
            scripts_dir, "validate_file_formats", 
...
class TestShellScriptHelp:
    ...
    def test_help_function(self, scripts_dir, script_name):
        ...
        assert result.returncode == 0, f"Help function failed for {script_name}: {result.stderr}"
...
class TestShellScriptIntegration:
    ...
    def test_fastp_file_validation(self, scripts_dir, sample_test_files, temp_test_dir):
        ...
        result = subprocess.run(
            ["bash", str(scripts_dir / "foli_01_fastp.sh"), fastq_files, str(output_dir)],
            capture_output=True,
            text=True,
            cwd=scripts_dir
        )
        ...
        result = subprocess.run(
            ["bash", str(scripts_dir / "foli_01_fastp.sh"), invalid_files, str(output_dir)],
            capture_output=True,
            text=True,
            cwd=scripts_dir
        )
    ...
    def test_count_file_validation(self, scripts_dir, sample_test_files, temp_test_dir):
        ...
        result = subprocess.run(
            ["bash", str(scripts_dir / "foli_04_count.sh"), alignment_files, str(output_dir)],
            capture_output=True,
            text=True,
            cwd=scripts_dir
        )
        ...
        result = subprocess.run(
            ["bash", str(scripts_dir / "foli_04_count.sh"), invalid_files, str(output_dir)],
            capture_output=True,
            text=True,
            cwd=scripts_dir
        )
...
class TestShellScriptRobustness:
    ...
    def test_sample_name_extraction_edge_cases(self, scripts_dir):
        ...
        edge_cases = [
            ...
            ("very_long_sample_name_with_many_underscores_R1.fastq.gz", "very_long_sample_name_with_many_underscores"),
        ...
        for input_file, expected_sample in edge_cases:
            ...
            assert stdout == expected_sample, f"Expected '{expected_sample}', got '{stdout}' for {input_file}"
